...
public:
     ...
     */
    void setOrientation(const Matrix3x3 m) { this->_orientMatrix = m; }
    ...
    int getVertexesNumber()const{return _vertexes.size();}
    /*!
    ...
    GeometricObj(const GeometricObj& Obj):_midPoint(Obj.getMidPoint()), _orientMatrix(Obj.getOrientation())
    {
        
        for(auto i = 0; i < Obj.getVertexesNumber(); ++i)
            _vertexes.push_back(Obj.getVertex(i));
            
    }

    GeometricObj(GeometricObj&& Obj):_midPoint(Obj.getMidPoint()), _orientMatrix(Obj.getOrientation())
    {
        for(auto i = 0; i < Obj.getVertexesNumber(); ++i)
            _vertexes.push_back(Obj.getVertex(i));
        Obj.ClearMe();
    ...
    GeometricObj& operator=(const GeometricObj& Obj)
    {
        _midPoint = Obj.getMidPoint();
        _orientMatrix = Obj.getOrientation();
                for(auto i = 0; i < Obj.getVertexesNumber(); ++i)
            _vertexes.push_back(Obj.getVertex(i));
        return *this;
    }
    GeometricObj& operator=( GeometricObj&& Obj)
    {
        _midPoint = Obj.getMidPoint();
        _orientMatrix = Obj.getOrientation();
                for(auto i = 0; i < Obj.getVertexesNumber(); ++i)
    ...
    // virtual GeometricObj(const GeometricObj &Obj) = 0;
